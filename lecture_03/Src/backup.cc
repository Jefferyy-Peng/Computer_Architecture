...
namespace simple
{
	...
	const uint32_t	latencies::L2  =   latencies::L1 * 2;		// L2 cache latency
	const uint32_t	latencies::L3  =   latencies::L2 * 2;		// L3 cache latency
	...
	const uint32_t	params::L2::nsets = 64;		// L2 number of sets
    const uint32_t	params::L2::nways = 4;		// L2 number of ways
    const uint32_t	params::L2::linesize = 8;	// L2 line size (bytes)
	const uint32_t	params::L3::nsets = 64 * 4;		// L3 number of sets
    const uint32_t	params::L3::nways = 4;		// L3 number of ways
    const uint32_t	params::L3::linesize = 8;	// L3 line size (bytes)
	...
	else if(caches::L2.hit(EA)) cycles += (latencies::L1+latencies::L2);
	...
	if (L1_hit) cycles += (latencies::L1+latencies::L2);
	else if(L2_hit) cycles += (latencies::L1+latencies::L2);
	else                    cycles += latencies::MEM;
    }
