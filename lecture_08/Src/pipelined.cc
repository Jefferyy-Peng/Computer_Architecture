...
namespace pipelined
{
    ...
    std::vector<PT>	PTable(params::PT::N);
    u32 			PRF::next = 0;
	...
	*((u8*)(data.data() + offset)) = B;
	...
	uint64_t	counters::wrong_pred = 0;
	uint64_t	counters::lastaddr = 0;
	bool		counters::taken = 0;
	u32			counters::this_latency = 0;
	...
	counters::lastcompleted = 0;
	counters::wrong_pred = 0;
	counters::lastaddr = 0;
	counters::taken = 0;
	counters::this_latency = 0;
	PRF::next = 0;
	...
	for (u32 i=0; i<params::PT::N; i++)  PTable[i].remove();
	units::FXU.clear();
	units::FPU.clear();
	units::LDU.clear();
	units::STU.clear();
	...
	{ 
		bool last_taken = counters::taken;
	    inst->count() = counters::instructions;
	    counters::instructions++;
	    bool correct = inst->fetch();	// fetch time
	    inst->decode();	// decode time
	    inst->dispatch();	// dispatch time
		if ((!correct)){ 
			inst->penalty(counters::lastcompleted + counters::this_latency);
			counters::lastfetch = inst->get_fetched();
		}
	    if (tracing) inst->output(std::cout);
	    bool taken = inst->process();
		counters::taken = taken;
